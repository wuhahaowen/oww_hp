    ...
    return (
        <div className="light-control-container flex-col">
          ...
          <span className="light-control-title">{lightName}</span>
            ...
            <span className="light-control-brightness-label"
                  style={{ marginLeft: '2.78vw', fontSize: '1.33vw' }}>亮度调节</span>
            <div className="light-control-brightness-wrapper flex-row justify-between">
                <img alt="" className="light-control-brightness-icon-left" src={imageAssets.lighting.lightLow}
                     style={{ marginLeft: '2.78vw' }}/>
                <div className="scrollbar-container">
                    <div className="scrollbar-track"></div>
                    <div className="scrollbar-fill brightness-fill" style={{ width: `${brightnessPercent}%` }}></div>
                    <div className="scrollbar-handle" style={{ left: `calc(${brightnessPercent}% - 0.695vw)` }}></div>
                    <input
                        type="range"
                        className="light-control-scrollbar"
                ...
                <img alt="" className="light-control-brightness-icon-right" src={imageAssets.lighting.lightHigh}
                     style={{ marginRight: '2.78vw' }}/>
            </div>

            <div className="light-control-brightness-value"
                 style={{ marginLeft: `calc(${brightnessPercent}% - 1.5vw + 2.78vw - 0.415vw)` }}>
            ...
            <span className="light-control-temp-label"
                  style={{ marginLeft: '2.78vw', fontSize: '1.33vw' }}>色温调节</span>
            <div className="light-control-temp-wrapper flex-row justify-between">
                ...
                <div className="scrollbar-container">
                    <div className="scrollbar-track"></div>
                    <div className="scrollbar-fill temp-fill" style={{ width: `${colorTempPercent}%` }}></div>
                    <div className="scrollbar-handle" style={{ left: `calc(${colorTempPercent}% - 0.695vw)` }}></div>
                    <input
                        type="range"
                        className="light-control-scrollbar"
                        ...
                        onChange={( e ) => {
                            ...
                            const minK = entity.attributes?.min_color_temp_kelvin || 2700;
                            const maxK = entity.attributes?.max_color_temp_kelvin || 6500;
                            const kelvin = Math.round( maxK - ( value * ( maxK - minK ) ) / 100 );
                            entity.service.turn_on( { serviceData: { color_temp_kelvin: kelvin } } );
                        }}
                        disabled={entity.state !== 'on'}
                        min={0}
                        max={100}
            ...
            <div className="light-control-temp-value"
                 style={{ marginLeft: `calc(${colorTempPercent}% - 1.5vw + 2.78vw - 0.415vw)` }}>
                <span className="light-control-temp-number">{colorTempPercent} </span>
                <span className="light-control-temp-percent">%</span>
            </div>
            ...
            <div className="light-control-effects-wrapper">
                {entity.attributes?.effect_list?.map( ( effect, index ) => (
                    <div className={`light-control-effect-item ${entity.attributes?.effect === effect ? 'active' : ''}`}
                         key={index} onClick={() => handleEffectChange( effect )}>
                        <div className="light-control-effect-content">
                            ...
                            />
                            <span
                                className="light-control-effect-text">{effect.length > 4 ? effect.slice( 0, 4 ) + '.' : effect}</span>
                        </div>
                    </div>
                ) )}
            </div>

            <div className={`light-control-divider flex-col ${entity.state === 'on' ? 'active' : ''}`}
